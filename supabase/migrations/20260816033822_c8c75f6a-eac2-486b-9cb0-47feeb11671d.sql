-- Final RLS policies for storage and designer role
do $migration$
declare
  definition text;
...
drop policy if exists "Importers manage own project import sessions" on public.project_import_sessions;
...
-- RLS for commercial-documents bucket
drop policy if exists "Company users can read commercial documents" on storage.objects;
create policy "Company users can read commercial documents"
...
drop policy if exists "Commercial roles can upload documents" on storage.objects;
create policy "Commercial roles can upload documents"
on storage.objects for insert to authenticated
with check (
  bucket_id = 'commercial-documents'
  and (storage.foldername(name))[1] = public.current_company_id()::text
