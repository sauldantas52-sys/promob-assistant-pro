import { ShieldCheck, Layers, Settings as Tool, AlertCircle, Box, Printer } from "lucide-react";
...
export function PreliminaryCutPlanTab({ projectId }) {
    const [integrityStatus, setIntegrityStatus] = useState('validating');
    ...
    const validateIntegrity = (groups, targetProjectId) => {
        const errors = [];
            ...
            group.sheets.forEach(sheet => {
                sheet.shelves.forEach(shelf => {
                    ...
                    shelf.placements.forEach(p => {
                        if (p.x < 5 || p.y < 5)
                            errors.push(`Placement invadiu margem esquerda/topo: ${p.physicalId}`);
                        if (p.x + p.w > 2745)
                            errors.push(`Placement invadiu margem direita: ${p.physicalId}`);
                        if (p.y + p.h > 1825)
                            errors.push(`Placement invadiu margem inferior: ${p.physicalId}`);
                    });
                });
            });
        });
    ...
    if (isLoading || partsLoading)
        return <div className="p-8 text-center text-xs text-slate-500 font-bold uppercase tracking-widest animate-pulse">Calculando Nesting Industrial Real...</div>;
    ...
    const excluded = allParts?.filter(p => !cutParts.find(cp => cp.id === p.id) &&
        !hardware.find(h => h.id === p.id) &&
        !accessories.find(a => a.id === p.id)) || [];
    const renderCutGroup = (group) => {
        ...
        return (<Card key={group.groupKey} className="overflow-hidden border-2 border-slate-200 shadow-sm mb-6">
        ...
        <CardContent className="p-6 space-y-8 bg-slate-50">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sheets.map((sheet, idx) => (<div key={idx} className="space-y-2">
                ...
                <div className="relative bg-white border-2 border-slate-300 rounded shadow-inner overflow-hidden" style={{ aspectRatio: '2750/1830', width: '100%' }}>
                  <div className="absolute border border-dashed border-slate-300 pointer-events-none z-10" style={{ left: '0.18%', top: '0.27%', right: '0.18%', bottom: '0.27%' }}/>
                  {sheet.shelves.flatMap(s => s.placements).map((p, pIdx) => (<div key={pIdx} className="absolute border border-slate-900 flex flex-col items-center justify-center overflow-hidden hover:opacity-80 transition-opacity cursor-help p-0.5" title={`${p.piece.name} (${p.w}x${p.h})`} style={{
                        left: `${(p.x / 2750) * 100}%`,
                        top: `${(p.y / 1830) * 100}%`,
                        width: `${(p.w / 2750) * 100}%`,
                        height: `${(p.h / 1830) * 100}%`,
          ...
          <div className="bg-white border rounded-lg overflow-hidden shadow-sm">
            <Table>
              ...
              <TableBody>
                {pieces.map((part) => (<TableRow key={part.physicalId} className="text-[10px]">
    ...
    return (<div className="space-y-8 max-w-[1400px] mx-auto p-4 lg:p-0 print:p-0">
      <div className="no-print">
        {integrityStatus === 'fail' ? (<Alert variant="destructive" className="rounded-xl border-2 border-red-500 bg-red-50 shadow-sm mb-6">
          ...
          </Alert>) : (<Alert className="rounded-xl border-2 border-blue-500 bg-blue-50 text-blue-900 shadow-sm mb-6">
      ...
      <Tabs defaultValue="plano" className="w-full no-print">
        ...
        <TabsContent value="plano" className="mt-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            {cutPlanGroups?.map(group => (<Card key={group.groupKey} className="border-2 border-slate-100 shadow-none">
