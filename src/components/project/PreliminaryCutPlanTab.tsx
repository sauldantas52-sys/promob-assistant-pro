...
export function PreliminaryCutPlanTab({ projectId }: { projectId: string }) {
  ...
  const renderCutGroup = (thickness: string, groupParts: typeof cutParts) => {
    ...
    return (
      <Card key={thickness} className="overflow-hidden border-2 border-slate-200 shadow-sm mb-6">
        <CardHeader className="bg-slate-900 text-white py-4 px-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-sm font-black uppercase tracking-tighter">MDF {thickness}</CardTitle>
            ...
            <div className="text-right">
              <p className="text-[9px] font-black text-lime-400 uppercase tracking-widest">Área Consolidada</p>
              <p className="text-xl font-black">{totalArea.toFixed(2)} m²</p>
            </div>
          </div>
        </CardHeader>
          ...
          <Table>
            <TableHeader className="bg-slate-50">
              <TableRow className="hover:bg-transparent">
                <TableHead className="text-[9px] font-black uppercase px-6">Descrição / UID</TableHead>
                ...
                <TableHead className="text-[9px] font-black uppercase text-center">Qtd / Rep</TableHead>
                <TableHead className="text-[9px] font-black uppercase text-right px-6">Área Unit.</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {groupParts.map((part) => {
                const area = ((part.width_mm || 0) * (part.length_mm || 0)) / 1000000;
                return (
                  <TableRow key={part.id} className="text-[11px] hover:bg-slate-50 border-slate-100">
                    <TableCell className="px-6 py-3">
                      <p className="font-black text-slate-900 uppercase">{part.name}</p>
                      <p className="text-[9px] font-mono text-slate-400">UID: {(part.metadata as any)?.unique_id || '-'}</p>
                    </TableCell>
                    <TableCell>
                      <Badge variant="outline" className="text-[9px] font-bold border-slate-200 text-slate-600 bg-white">
                        {part.material || "NÃO INFORMADO"}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-center font-mono font-bold text-slate-700">
                      {part.width_mm} x {part.length_mm}
                    </TableCell>
                    <TableCell className="text-center">
                      <span className="font-black text-slate-900">{part.quantity}</span>
                      <span className="text-slate-400 mx-1">/</span>
                      <span className="text-slate-500">{(part.metadata as any)?.repetition || 1}</span>
                    </TableCell>
