...
export function IndustrialLabelsTab({ pieces }) {
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-xl border-2 border-slate-100 shadow-sm no-print">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" className="text-[10px] font-black uppercase tracking-widest gap-2" onClick={() => setShowSettings(!showSettings)}>
            <Settings className="h-3.5 w-3.5"/> Ajustes
          </Button>
          <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-black uppercase tracking-widest gap-2" onClick={handlePrint}>
            <Printer className="h-3.5 w-3.5"/> Imprimir Tudo
          </Button>
        </div>
      </div>

      {showSettings && (<Card className="border-2 border-indigo-100 shadow-none no-print">
          <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="space-y-2">
              ...
              <Select value={selectedPreset} onValueChange={handlePresetChange}>
                ...
                <SelectContent>
                  {PRESETS.map(p => (<SelectItem key={p.id} value={p.id} className="text-xs font-bold uppercase">{p.name}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input type="number" value={customConfig.width} onChange={e => setCustomConfig({ ...customConfig, width: Number(e.target.value) })} className="h-9 text-xs font-bold"/>
            </div>

            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Altura (mm)</Label>
              <Input type="number" value={customConfig.height} onChange={e => setCustomConfig({ ...customConfig, height: Number(e.target.value) })} className="h-9 text-xs font-bold"/>
            </div>

            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Colunas</Label>
              <Input type="number" value={customConfig.cols} onChange={e => setCustomConfig({ ...customConfig, cols: Number(e.target.value) })} className="h-9 text-xs font-bold"/>
            </div>
          </CardContent>
        </Card>)}

      {/* Grid de Etiquetas */}
      <div className="grid gap-4 print:gap-0 print:grid-cols-[repeat(var(--cols),1fr)]" style={{
            '--cols': customConfig.cols,
            display: 'grid',
            gridTemplateColumns: `repeat(${customConfig.cols}, 1fr)`
        }}>
        {sortedPieces.map((piece) => (<IndustrialLabel key={piece.physicalId} piece={piece} width={customConfig.width} height={customConfig.height}/>))}
      </div>
...
function IndustrialLabel({ piece, width, height }) {
    ...
    return (<div className="bg-white border border-slate-200 relative p-3 overflow-hidden print:border-slate-300 print:shadow-none shadow-sm rounded-lg print:rounded-none flex flex-col justify-between" style={{
            width: `${width}mm`,
            height: `${height}mm`,
      ...
      <div className="flex justify-between items-center flex-1 py-1">
        <div className="flex-1 space-y-0.5 pr-2">
          ...
          {piece.metadata?.observations && (<p className="text-[7px] font-black uppercase text-red-600 leading-none bg-red-50 p-0.5 rounded mt-1">
              LEGENDA: {piece.metadata.observations}
            </p>)}
        </div>

        <div className="flex-shrink-0">
          <QRCodeSVG value={data.qrPayload} size={qrSize * 3.78} // mm to px approx
     level="L" includeMargin={false}/>
        </div>
      </div>
...
function EdgeSchema({ piece }) {
    return (<div className="relative w-6 h-4 border border-slate-200 bg-slate-50 rounded-[1px]">
      {/* Top Edge */}
      {piece.edgeTop > 0 && (<div className="absolute top-0 left-0 right-0 h-[2px]" style={{ backgroundColor: getEdgeColor(piece.edgeNameGeneral) }}/>)}
      {/* Bottom Edge */}
      {piece.edgeBottom > 0 && (<div className="absolute bottom-0 left-0 right-0 h-[2px]" style={{ backgroundColor: getEdgeColor(piece.edgeNameGeneral) }}/>)}
      {/* Left Edge */}
      {piece.edgeLeft > 0 && (<div className="absolute top-0 bottom-0 left-0 w-[2px]" style={{ backgroundColor: getEdgeColor(piece.edgeNameGeneral) }}/>)}
      {/* Right Edge */}
      {piece.edgeRight > 0 && (<div className="absolute top-0 bottom-0 right-0 w-[2px]" style={{ backgroundColor: getEdgeColor(piece.edgeNameGeneral) }}/>)}
    </div>);
}
