...
import { ArrowLeft, Upload, Loader2, AlertTriangle, Boxes, Layers, Wrench, FileText, Download, } from "lucide-react";
...
export const Route = createFileRoute("/projects/$projectId")({
    head: () => ({
        meta: [
            ...
            {
                name: "description",
                content: "Módulos, peças, chapas e ferragens extraídos do arquivo do Promob, prontos para orçamento, produção e conferência.",
            },
...
function ProjectDetail() {
        ...
        try {
            ...
            for (const parsedModule of result.modules) {
                ...
                if (parsedModule.parts.length > 0) {
                    const { error: partsError } = await supabase.from("parts").insert(parsedModule.parts.map((part) => ({
                        project_id: projectId,
            ...
            toast.success(result.modules.length > 0
    ...
    const totalArea = panels.reduce((sum, p) => sum + ((p.width_mm ?? 0) / 1000) * ((p.length_mm ?? 0) / 1000) * Number(p.quantity ?? 1), 0) || 0;
    const exportCSV = () => {
        try {
            const opts = {};
            const parser = new Parser(opts);
            const csv = parser.parse(allParts.map((p) => ({
    ...
    return (<div className="space-y-6 p-4 md:p-8">
      ...
      <Card className="border-primary/20">
        ...
        <CardContent className="space-y-3">
          ...
          <input ref={fileInput} type="file" accept=".xml,.pdf,.dxf" className="hidden" onChange={(e) => {
          ...
          {warnings.length > 0 && (<div className="space-y-2 rounded-lg border border-destructive/30 bg-destructive/5 p-3">
              {warnings.map((warning) => (<p key={warning} className="flex gap-2 text-sm text-destructive">
          ...
          <Card>
            <CardContent className="p-0">
              <Accordion type="multiple" className="w-full">
                {(modules.data ?? []).map((m) => (<AccordionItem key={m.id} value={m.id} className="border-b px-4 py-1 last:border-0">
                    <div className="flex items-center gap-3">
                      <Checkbox checked={m.is_completed ?? false} onCheckedChange={async (checked) => {
                const { error } = await supabase
                    .from("modules")
                ...
                .map((p) => (<TableRow key={p.id} className="hover:bg-transparent">
                                  <TableCell className="py-2 text-xs font-medium">
                                    <div className="flex items-center gap-2">
                                      <Checkbox checked={p.is_completed ?? false} className="h-3.5 w-3.5" onCheckedChange={async (checked) => {
                    const { error } = await supabase
                        .from("parts")
                        .update({ is_completed: !!checked })
                        .eq("id", p.id);
                    if (error)
                        ...
                        void queryClient.invalidateQueries({ queryKey: ["parts", projectId] });
                            ...
                            {allParts.filter((p) => p.module_id === m.id).length === 0 && (<TableRow>
                ...
                {(modules.data?.length ?? 0) === 0 && (<div className="py-12 text-center text-sm text-muted-foreground">
          ...
          <Card>
            <CardContent className="overflow-x-auto p-0">
              <Table>
                ...
                <TableBody>
                  {allParts.map((p) => (<TableRow key={p.id}>
        ...
        <TabsContent value="files">
          <Card>
            <CardContent className="divide-y p-0">
              {(files.data ?? []).map((f) => (<div key={f.id} className="flex items-center justify-between gap-3 p-4">
                  <div>
              ...
              {(files.data?.length ?? 0) === 0 && (<p className="p-6 text-sm text-muted-foreground">Nenhum arquivo importado ainda.</p>)}
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
